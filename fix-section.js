      ...
      try {
        ...
        if (isValid) {
          ...
          if (attempts >= maxAttempts) {
            playNotification('Секция ' + section.title + ' сгенерировалась пустой. Попробуйте нажать кнопку "Перегенерировать секцию".', 'error');
          } else {
        ...
        if (attempts >= maxAttempts) {
          playNotification('Ошибка при генерации сценария. Попробуйте еще раз.', 'error');
        } else {
          await new Promise(resolve => setTimeout(resolve, 2000));
        }
      }
